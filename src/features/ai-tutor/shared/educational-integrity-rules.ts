...
export function transformAnswerToGuidance(
  content: string,
  context?: { topic?: string; question?: string },
): string {
  const topic = context?.topic?.trim();
  const question = context?.question ?? content;
  const base = buildGuidedLearningResponse(question);
...
export function buildAssessmentFallbackSuggestions(
  ...
  if (isLikelyEnglish(question)) {
    return [
      ...
      'I can explain the concept this question is testing — tell me which topic feels unclear.',
      'Share your current reasoning and I will point out gaps without giving the final answer.',
    ];
  }

  return [
    buildGuidedLearningResponse(question),
    'يمكنني شرح المفهوم الذي يختبره السؤال — أخبرني أي موضوع غير واضح لديك.',
    'شاركني تفكيرك الحالي وسأشير إلى الثغرات دون إعطاء الإجابة النهائية.',
  ];
}
