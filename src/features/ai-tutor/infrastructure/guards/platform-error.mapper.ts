...
export function mapPlatformErrorToAskTutorError(
  ...
  const code =
    error instanceof PlatformError ? error.code : error.code;
  const message =
    error instanceof PlatformError ? error.message : error.message;

  switch (code) {
    case PlatformErrorCodes.RATE_LIMITED:
      return new AskTutorError(429, message, AskTutorErrorCodes.RATE_LIMIT_EXCEEDED);
    ...
    case PlatformErrorCodes.COST_CAP_EXCEEDED:
      return new AskTutorError(503, message, AskTutorErrorCodes.SERVICE_UNAVAILABLE);
