...
const DIRECT_ANSWER_RESPONSE_PATTERNS: Array<{
  ...
  { pattern: /\bthe\s+correct\s+answer\s+is\b/i, severity: 'high', description: 'States the correct answer directly' },
  { pattern: /\bthe\s+answer\s+is\s+(option\s+)?[A-D]\b/i, severity: 'high', description: 'Reveals a multiple-choice answer' },
  { pattern: /\bcorrect\s+option\s*(is|:)\s*[A-D]\b/i, severity: 'high', description: 'Reveals the correct option letter' },
  { pattern: /\banswer\s*key\b/i, severity: 'high', description: 'References an answer key' },
  { pattern: /\bfinal\s+solution\s*(is|:)\b/i, severity: 'medium', description: 'Provides a final solution framing' },
  { pattern: /الإجابة\s*الصحيحة\s*(هي|:)/, severity: 'high', description: 'States the correct answer directly (Arabic)' },
  { pattern: /الخيار\s*(الصحيح)?\s*(هو|:)?\s*[أ-دA-D]/, severity: 'high', description: 'Reveals the correct option (Arabic)' },
  { pattern: /مفتاح\s*الإجابة/, severity: 'high', description: 'References an answer key (Arabic)' },
  { pattern: /الحل\s*النهائي\s*(هو|:)/, severity: 'medium', description: 'Provides a final solution framing (Arabic)' },
];
...
export function validateEducationalResponse(response: string): ResponseIntegrityResult {
